...
dfr.SD<-selectStrata.TrawlSurvey(tmp0,
                                 ...
                                 export=FALSE,
                                 verbosity=verbosity) |>
          dplyr::filter(nchar(GIS_STATION)<6);#--exclude 6-digit "corner" stations
...
#--write.csv(tmp,file.path(dirThs,"csv_dfrACD.BySexGroup.PribDistrict.csv"),row.names=FALSE);
...
tsr=list(dirData=dirData,
         ...
         evs.stns=evs.stns,
         evs.grid=evs.grid);
wtsUtilities::saveObj(tsr,fn=file.path(dirThs,"rda_wts.PIBKC.NMFS.NoCornerStations.RData"));
