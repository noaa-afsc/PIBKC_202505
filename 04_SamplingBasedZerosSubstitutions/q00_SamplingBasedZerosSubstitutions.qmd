---
title: "A sampling distribution-based method for replacing zero catches in time series for REMA fitting"
...
Mature male abundance and biomass of Pribilof Islands blue king crab in the annual NMFS EBS bottom trawl survey has declined substantially since the survey was initiated in 1975 ([@fig-Zeros-SrvAbdBio]), including zero catches at 86 and 70 survey stations in, respectively, the 2023 and 2024 surveys. At low abundance, abundance appears to scale exponentially with the number of hauls at which mature males were caught ([@fig-Zeros-Non0sByAbd]). The existence of zeros in the corresponding time series for design-based estimates of mature male biomass within the Pribilof Islands survey strata ([@fig-Zeros-SurveyBasemap]) poses methodological difficulties for time series modeling approaches like `rema` [@rema] that model variability on the log scale. `rema` provides three options for dealing with zeros in an observed time series: 1) treat the zeros as missing data; 2) substitute a small, but arbitrary, value for the zeros; or 3) assume an error distribution based on the Tweedie distribution. For the latter two options (2, 3), an associated error CV (coefficient of variation) must also be specified. Lognormal error distributions are assumed under the first two options (1,2). Using the Tweedie distribution (option 3) has the advantage that it can synthetically fit zeros in the data, but at the cost of estimating an extra parameter (related to the variance) and issues with convergence when optimizing the model (finding the maximum likelihood) may occur.

```{r}
#| label: fig_Zeros-SrvAbdBio
#| results: asis
#| fig-width: 6.5 
#| fig-asp: 1.0
...
#| fig-asp: 1.0
  ...
  p = ggplot() + basemap +
        ...
        geom_sf(data=lstSrv$evs.stns |> dplyr::filter(YEAR==2023),colour="blue",alpha=0.9);# + 
...
$$ Pr(N=0|\text{K hauls}) = \prod_K e^{-\lambda} = e^{-K \cdot \lambda} $$ {#eq-PoissDist}

Setting $p=Pr(N=0|\text{K hauls})$ and rearranging to solve for the density, $d$, one obtains:

$$d=-\frac{ln(p)}{K \cdot A_s} $$ {#eq-Density}
...
```{r}
#| label: fig_Zeros-EstAbdWithNDPs
#| results: asis
#| fig-width: 6.5 
#| fig-asp: 0.5
...
```{r}
#| label: fig_Zeros-EstBioWithNDPs
#| results: asis
#| fig-width: 6.5 
#| fig-asp: 0.5
  p = ggplot(dfp,aes(x=year)) +
    ...
    if (!is.null(dfrLast)){
      p1 = p1 +
            geom_ribbon(data=dfrLast,mapping=aes(x=year,y=pred,ymin=pred_lci,ymax=pred_uci),
                        colour=NA,fill="red",alpha=0.2,inherit.aes=FALSE) +
...
```{r Zeros_Make_Tbl_REMA_Models}
  ...
  dfrRTs = tibble::tribble(~model,~`error distribution`,~`0's handling`,~`run time (s)`,~`max gradient`,
                            opt1,"lognormal","0's treated as missing (NA's)",
                                 lstAll$remas[[opt1]]$rema_mdl$runtime,max(abs(lstAll$remas[[opt1]]$rema_mdl$final_gradient)),
                            opt2,"lognormal","small constant (0.01) substituted with large CV",
                                 lstAll$remas[[opt2]]$rema_mdl$runtime,max(abs(lstAll$remas[[opt2]]$rema_mdl$final_gradient)),
                            opt3,"Tweedie","0 included, but with large CV",
                                 lstAll$remas[[opt3]]$rema_mdl$runtime,max(abs(lstAll$remas[[opt3]]$rema_mdl$final_gradient)),
                            opt4,"lognormal","Poisson-based; non-detection p = 0.5",
                                 lstAll$remas[[opt4]]$rema_mdl$runtime,max(abs(lstAll$remas[[opt4]]$rema_mdl$final_gradient)),
                            opt5,"Tweedie","Poisson-based; non-detection p = 0.5",
                                 lstAll$remas[[opt5]]$rema_mdl$runtime,max(abs(lstAll$remas[[opt5]]$rema_mdl$final_gradient)));
```

Five `rema` models were run to compare results from using its standard options for dealing with time series 0's to the proposed distribution-based approach ([@tbl-Zeros-REMA-Models]). The model that assumed a Tweedie error distribution and used the $p = 0.5$ non-detection criterion may not have converged (the max gradient was `r formatC(lstAll$remas[["Tweedie w/ p=0.5"]]$rema_mdl$final_gradient[2])` while the variance parameter was estimated essentially at its arithmetic upper bound (estimate = `r formatC(tw_p,digits=6)`, upper bound = 2). In addition, it took `r as.numeric(dfrRTs$time[5])` seconds for this model to converge as compared with < 1s for the other models.  

```{r tbl_Zeros-REMA-Models}
#| results: asis
  flx = flextable::as_flextable(dfrRTs |> dplyr::mutate(`max gradient`=formatC(`max gradient`,format="e")), 
                                show_coltype=FALSE,max_row=100) |> 
          flextable::delete_part("footer")|> 
          flextable::font(fontname="Times New Roman",part="all") |>
          flextable::fit_to_width(9);
  cap = paste0("`rema` models, with timing and maximum gradient as reported for model convergence.");
  lstTbls = c(lstTbls,
              insertFlextableIntoQMD(flx,
                                     lbl="tbl-Zeros-REMA-Models", 
                                     cap=cap,
                                     ori="L"));
```

The results from the five models are compared in Figures [-@fig-Zeros-CompareAll1]-[-@fig-Zeros-ComparePs2]. Results from both the lognormal model that treated the 0's as missing values and the Tweedie model that included the 0's but assigned a large CV to the values were notable in that the estimated values for 2023 and 2024 were the same as the value for 2022, but with increasing uncertainty bounds. Results from the model that assumed a lognormal error distribution and substituted a small constant for the observed 0's stand out from the others both for the lager uncertainty intervals across the time series as well the substantial departure of its estimates in 2022-2024 from the others (particularly noticeable on the log-10 scale, Figures [-@fig-Zeros-CompareAll2] and [-@fig-Zeros-CompareLNs2]). Of the five, the lognormal error model that used the non-detection criterion exhibited the most satisfactory results: it fit the last non-zero observation better than any of the other models and did not exhibit any really questionable features. MCMC diagnostics for this model appear to be reasonable ([@fig-Zeros-MCMCDiags]). One-step ahead (OSA) residuals diagnostics ([@fig-Zeros-OSAs]) suggest that the model tends to overestimate the observed values. However, the diagnostics for this model are similar to those for the lognormal model that treats 0's as missing and much better than the lognormal model that substitutes a small constant for the 0's (OSAs are not yet available for Tweedie models).

```{r}
#| label: fig_Zeros-CompareAll1
#| results: asis
#| fig-width: 6.5 
#| fig-asp: 1.0
  ##--compare all----
  ...
  lbl = wtsQMD::getLabel();
  cap = paste0("Comparison of the `rema` results from fitting the PIBKC survey biomass time series for ",
               "mature males using the five error distributions and 0-substitution methods ",
               "discussed in the text. Full time series results. ",
               "Upper figure: arithmetic scale; lower figure: log-10 scale.")
  pth = wtsQMD::getFigFN();
  lstFigs = c(lstFigs,wtsQMD::printGGplot(p,lbl=lbl,cap=cap,pth=pth,ori="P"));
```

```{r}
#| label: fig_Zeros-CompareAll2
#| results: asis
#| fig-width: 6.5 
#| fig-asp: 1.0
  ##--compare all----
  ...
  cap = paste0("Comparison of the `rema` results from fitting the PIBKC survey biomass time series for ",
               "mature males using the five error distributions and 0-substitution methods ",
               "discussed in the text. Only results starting in 2010 to show detail at end of time series. ",
               "Upper figure: arithmetic scale; lower figure: log-10 scale.")
  pth = wtsQMD::getFigFN();
  lstFigs = c(lstFigs,wtsQMD::printGGplot(p,lbl=lbl,cap=cap,pth=pth,ori="P"));
```
```{r}
#| label: fig_Zeros-CompareLNs1
#| results: asis
#| fig-width: 6.5 
#| fig-asp: 1.0
  ##--compare lognormals----
  dfrs = lstAll$dfrPrds |> dplyr::filter(option %in% c(opt1,opt2,opt4))
  p = compareOpts(dfrs,ymax=50000,
                  dfrLast=lstAll$dfrSrvMMB_last);
  lbl = wtsQMD::getLabel();
  cap = paste0("Comparison of the `rema` results from fitting the PIBKC survey biomass time series for ",
               "mature males using the lognormal error distributions and 0-substitution methods ",
               "discussed in the text. Full time series results. ",
               "Upper figure: arithmetic scale; lower figure: log-10 scale.")
  pth = wtsQMD::getFigFN();
  lstFigs = c(lstFigs,wtsQMD::printGGplot(p,lbl=lbl,cap=cap,pth=pth,ori="P"));
...
#| fig-asp: 1.0
  ...
  lbl = wtsQMD::getLabel();
  cap = paste0("Comparison of the `rema` results from fitting the PIBKC survey biomass time series for ",
               "mature males using the Tweedie error distributions and 0-substitution methods ",
               'discussed in the text. Note that the "p=0.5" model was probably not converged. ',
...
#| fig-asp: 1.0
  ...
  cap = paste0("Comparison of the `rema` results from fitting the PIBKC survey biomass time series for ",
               "mature males using the Tweedie error distributions and 0-substitution methods ",
               'discussed in the text. Note that the "p=0.5" model was probably not converged. ',
               'Only results starting in 2010 to show detail at end of time series. ',
               "Upper figure: arithmetic scale; lower figure: log-10 scale.")
  pth = wtsQMD::getFigFN();
  lstFigs = c(lstFigs,wtsQMD::printGGplot(p,lbl=lbl,cap=cap,pth=pth,ori="P"));
```
...
#| fig-asp: 1.0
  ...
  lbl = wtsQMD::getLabel();
  cap = paste0("Comparison of the `rema` results from fitting the PIBKC survey biomass time series for ",
               'mature males using the two error distributions and "p=0.5" substitution method ',
               'discussed in the text. Note that the Tweedie model was probably not converged. ',
               'Full time series results. ',
               "Upper figure: arithmetic scale; lower figure: log-10 scale.")
  pth = wtsQMD::getFigFN();
  lstFigs = c(lstFigs,wtsQMD::printGGplot(p,lbl=lbl,cap=cap,pth=pth,ori="P"));
...
```{r Zeros_RunPreferred}
  ##--run MCMC on best model----
  best = opt4;
  if (FALSE){
    dfrp = lstAll$remas[[best]]$dfrSrvMMB|> dplyr::select(year,value=obs,cv=obs_cv);
    bestREMA = runREMA(best,dfrp,zeros=lstAll$remas[[best]]$zeros,doMCMC=TRUE);
    wtsUtilities::saveObj(bestREMA,file.path(child_path$peek(),"rda_Zeros_REMA_Models.Best.RData"));
  } else {
    bestREMA = wtsUtilities::getObj(file.path(child_path$peek(),"rda_Zeros_REMA_Models.Best.RData"));
  }
```

```{r}
#| label: fig_Zeros-MCMCDiags
#| results: asis
#| fig-width: 6.5 
#| fig-asp: 1.0
  p = wtsMCMC::mcmcTMB_PlotParamDiagnostics(bestREMA$dfrPDs,
                                            paste0("biomass_pred[",bestREMA$tdy_mxIdx,"]"),
                                            label="terminal MMB");
  lbl = wtsQMD::getLabel();
  cap = paste0('MCMC diagnostics for the "',best,'" model.')
  pth = wtsQMD::getFigFN();
  lstFigs = c(lstFigs,wtsQMD::printGGplot(p,lbl=lbl,cap=cap,pth=pth,ori="P"));
```

```{r}
#| label: fig_Zeros-OSAs
#| results: asis
#| fig-width: 6.5 
#| fig-asp: 1.0
  plotOSAs<-function(osas){
      old_thm = ggplot2::theme_set(cowplot::theme_cowplot(font_size = 10) +
                                   cowplot::background_grid() +
                                   cowplot::panel_border());
      thm1 = ggplot2::theme(legend.position="inside",
                            legend.position.inside=c(0.01,0.99),
                            legend.justification.inside=c(0.01,0.99));
      thm = wtsPlots::getStdTheme();
      p1 = osas$plots$biomass_resids + thm;
      p2 = osas$plots$biomass_fitted + thm;
      p3 = osas$plots$biomass_qq     + thm + thm1;
      pg1 = cowplot::plot_grid(p1,p2,nrow=1);
      pg  = cowplot::plot_grid(pg1,p3,ncol=1);
      ggplot2::theme_set(old_thm);
      return(pg);
  }
  p = plotOSAs(bestREMA$osas);
  # plotOSAs(lstAll$remas[[opt1]]$osas);
  # plotOSAs(lstAll$remas[[opt2]]$osas);
  # plotOSAs(lstAll$remas[[opt3]]$osas);
  # plotOSAs(lstAll$remas[[opt4]]$osas);
  # plotOSAs(lstAll$remas[[opt5]]$osas);
  lbl = wtsQMD::getLabel();
  cap = paste0('OSA (one-step ahead) diagnostics for the "',best,'" model.')
  pth = wtsQMD::getFigFN();
  lstFigs = c(lstFigs,wtsQMD::printGGplot(p,lbl=lbl,cap=cap,pth=pth,ori="P"));
```
