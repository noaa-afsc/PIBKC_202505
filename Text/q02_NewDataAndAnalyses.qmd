---
title: "2025 PIBKC Assessment Considerations: New Data and Analyses"
author:
...
```{r}
fldr = "../03_NoCornerStations";
child_path$push(file.path(child_path$peek(),fldr));
```
```{r sct_NoCorners, child=file.path(child_path$peek(),"q00_NoCornerStations.qmd")}
```
```{r}
str = child_path$pop();
```
...
child_path$push(file.path(child_path$peek(),fldr));
```
```{r sct_sdmTMB, child=file.path(child_path$peek(),"q00_PIBKC_sdmTMB.qmd")}
```
```{r}
str=child_path$pop();
```
